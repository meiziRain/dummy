...
class Sm4Utils {
     ...
     */
    private static final int INT_ONE = 0x01;
    ...
    public static byte[] encrypt(byte[] plainBytes, byte[] secretKey, byte[] iv){
        ...
        {
            throw new RuntimeException("secretKey's length is wrong!");
        }

        if (iv.length != IV_SIZE)
        {
            throw new RuntimeException("iv's length is wrong!");
        }
    ...
    public static byte[] decrypt(byte[] cipherBytes, byte[] secretKey){
        ...
        {
            throw new RuntimeException("ciphertext's length is wrong!");
        }
        ...
        // To ensure that the padding of output_p is valid
        if(p > BLOCK_SIZE || p < INT_ONE)
        {
            throw new RuntimeException("There no exists such padding!");
        }
